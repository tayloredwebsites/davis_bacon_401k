class EmployeePackage < ActiveRecord::Base
  ...
  attr_accessible :employee_id, :eff_month, :eff_year, :employee_id, :monthly_medical, :hourly_wage, :annual_sick, :annual_holiday, :annual_vacation, :annual_personal, :safe_harbor_pct
...
public  ## following methods will be public
  ...
  def is_package_duplicate
    if self.id.present?
      emp_pkgs = EmployeePackage.where("eff_month = ? and eff_year = ? and employee_id = ? and id <> ? and deactivated = 0", self.eff_month, self.eff_year, self.employee_id, self.id ).count
    else
      emp_pkgs = EmployeePackage.where("eff_month = ? and eff_year = ? and employee_id = ? and deactivated = 0", self.eff_month, self.eff_year, self.employee_id ).count
    end
    if emp_pkgs > 0
