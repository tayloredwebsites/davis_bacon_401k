...
class AccountControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    @all_users = User.order('login').all
  end
  ...
  def test_auth_bob
    ...
    assert_not_nil :user
    assert_equal 1000001, @request.session[:user_id]
  end
  ...
  def test_signup
    ...
    assert_not_equal nil, @request.session[:user_id]
  ...
  def test_bad_signup
    ...
    @request.session[:return_to] = "/bogus/location"
    post :signup, :user => { :login => "newbob", :password => "newpassword", :password_confirmation => "newpassword" }
    assert_equal false, @request.session[:user_id].present?
